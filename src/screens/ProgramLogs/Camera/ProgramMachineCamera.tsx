...
import { findMachineAsync } from '../../../redux/Slice/productionSlice';
interface CamProps {
    navigation: any; // Replace 'any' with the appropriate navigation prop type if available
}

const ProgramMachineCamera = ({ navigation }: any) => {
        ...
        AsyncStorage.getItem('programMData')
            .then((data) => {
                if (data) {
                    ...
                    setScannedMData(scannedEDataFromStorage)
            ...
            AsyncStorage.getItem('scannedBarcodeMachineData')
            .then((data) => {
                if (data) {
            ...
            barcodes.forEach(async (scannedBarcode) => {
                if (scannedBarcode.rawValue && scannedBarcode.rawValue !== '') {
                    ...
                    dispatch(findMachineAsync(data)).then((response: any) => {
                        ...
                        AsyncStorage.setItem('scannedBarcodeMachineData', JSON.stringify(updatedScannedData))
                            .then(() => {
                        ...
                        const updatedScannedEData = [...scannedMData, response.payload]; // Update the state value
                        setScannedMData(updatedScannedEData);
                        AsyncStorage.setItem('programMData', JSON.stringify(updatedScannedEData))
                            .then(() => {
                                console.log('scannedEData stored in AsyncStorage');
                            })
                            .catch((error) => {
    ...
    const finishAllotment = () => {
        ...
        console.log("Scanned Slip Data", existingParams);
        navigation.navigate("SingleProgramLogs",{displace:existingParams});
    };
...
export default ProgramMachineCamera
