...
import { View, StyleSheet, StatusBar, Vibration, Alert, Button, Modal, Text, FlatList, TouchableOpacity, BackHandler, Image, Dimensions } from 'react-native';
...
const ScannedSlipCamera: React.FC<CamProps> = ({ navigation }) => {
    ...
    const baseUrl = 'https://chawlacomponents.com/api/v1/employee/getSingle/';
    useEffect(() => {
            ...
            barcodes.forEach(async (scannedBarcode) => {
                if (scannedBarcode.rawValue && scannedBarcode.rawValue !== '') {
                    ...
                    dispatch(findEmployeeAsync(data)).then((response: any) => {
                        console.log("Employee slip camera", response.payload.employee._id);
                        Toast.show({
                        ...
                        AsyncStorage.setItem('scannedEmployeesData', JSON.stringify(updatedScannedData))
                            .then(() => {
                        ...
                        AsyncStorage.setItem('scannedEData', JSON.stringify(updatedScannedEData))
                            .then(() => {
                                console.log('scannedEData stored in AsyncStorage');
                            })
                            .catch((error) => {
    ...
    const finishAllotment = () => {
        ...
        navigation.navigate("ScannedSlip", { ...existingParams, params1: scannedData, params5: scannedEData });
    };
...
export default ScannedSlipCamera;
